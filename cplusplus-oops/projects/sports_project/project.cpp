...
public:
    ...
    void Player_display_c_bt(vector<Batsman>& Bt)
    {
        ...
        for (const auto& batsman : Bt) {
            cout << left << setw(11) << batsman.First_name
                << setw(12) << batsman.Last_name
                << setw(4) << batsman.Age
                << setw(28) << batsman.Team
                << setw(13) << batsman.Country << endl;
            cout << "-------------------------------------------------------------------" << endl;
        }
        cout << endl;
    }
...
void cricketPlayerMenu(vector<Batsman>& Bt, vector<Bowler>& Bo, vector<All_rounder>& Ar, vector<Wicket_keeper>& Wk) {
    while (true) {
        ...
        if (player == 1) {
            ...
            if (!Bt.empty()) {
                Bt[0].Player_display_c_bt(Bt);
            }

            if (!Bo.empty()) {
                Bo[0].Player_display_c_bo(Bo);
            }

            if (!Ar.empty()) {
                Ar[0].Player_display_c_ar(Ar);
            }

            if (!Wk.empty()) {
                Wk[0].Player_display_c_wk(Wk);
            }
        }
        else if (player == 3) {
            return;
...
void cricketMenu(Cricket& C, vector<Batsman>& Bt, vector<Bowler>& Bo, vector<All_rounder>& Ar, vector<Wicket_keeper>& Wk) {
    while (true) {
        C.displaymain_cricket(); // Display the main cricket menu
        int switch_c;
        cin >> switch_c;
